...
class TestPythonFunctionPractice(unittest.TestCase):
  ...
  def test_volume_of_cube(self):
    length_of_cube_side = 5
    volume = cubic_volume(length_of_cube_side) 
    self.assertEqual( 125, volume )

  #Given a String, return the String reversed
  #@unittest.skip("delete this line to run the test")
  def test_reverse_string(self):
    input_string = "namtaB ma I"
    reversed_string = rev_string(input_string)
    self.assertEqual(reversed_string, "I am Batman")

  #Given a value in farenheit, convert this into celsius.
  #@unittest.skip("delete this line to run the test")
  def test_fahrenheit_to_celsius(self):
    faren = 120.0
    faren_to_cels = f_to_c(faren)
    self.assertEqual(faren_to_cels, 48.88888888888889)
